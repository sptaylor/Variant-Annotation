		...
		foreach my $tv(@{$new_vf->get_all_TranscriptVariations}) {
			...
			#foreach my $tva(@{$tv->get_all_alternate_TranscriptVariationAlleles}) {
				...
				if ( $config->{use_uniprot} && $con{uniprot_id} ) {
					get_overlapping_protein_features_from_UniProt( $config, \%con );
					get_overlapping_protein_features_from_UniProt_orthologues($config, \%con );
				}
				#debug("uniprot");
				
				if ( $config->{use_ncbi} ) {
					get_model_orthologue_info_from_NCBI( $config, \%con );
				}
				#debug("ncbi");
				...
				if ( ( $con{changes_protein} ) ) {
					...
					#foreach my $species(keys %$vf_hash)
					
					#debug("determine_aligned homolog residues");
					determine_alignment_score_change( $matrix, \%con );
					#debug("determine alignment score change");
					determine_reference_amino_acid_conservation( \%con );
					#debug("determine ref aa conservation");
					determine_reference_amino_acid_score( $matrix,\%con );
					#debug("determine ref aa score");
					#determine_context_average_percent_identity( $config,\%con );
					#debug("determine average percent identity");
				...
				#debug("determine model annotations ");
				my $other_an=determine_other_annotations( $config, \%con);
				#debug("determine other annotations ");
				
				#Information about protein site conservation
				
				my ($Amino_Acids_In_Orthologues, $Orthologue_Species, $Alignment_Score_Change, $C_blosum, $C_ident, $Model_Annotations, $GO, $interactions, $allele_frequencies, $allele_populations);
			...
			#	my $refalt_column;
				...
				while ($curidx lt $config->{number_of_samples}) {
					if ($info[$curidx] =~ /:/) {
...
sub get_model_orthologue_info_from_NCBI {
    ...
    $con->{entrez_gene_model_kegg_pathways} = $parsed->{kegg};
    $con->{entrez_gene_model_phenotypes}    = $parsed->{phenotypes};
    $con->{entrez_gene_model_interactions_count}= $parsed->{interaction_count};
    $con->{entrez_gene_model_reactome} = $parsed->{reactome};
    $con->{entrez_gene_model_summary} = $parsed->{entrez_gene_summary};
    #$con->{entrez_gene_model_other_source_anchor} = $parsed->{other_source_anchor};
    $con->{entrez_gene_model_gene_rif} = $parsed->{gene_rif};
...
#    my $other_source_anchor_re
#        ="\Q<Other-source_anchor>\E(.+)\Q</Other-source_anchor>\E";
#	
#	$results{other_source_anchor}=get_unique_re_matches( $other_source_anchor_re, $xml);
